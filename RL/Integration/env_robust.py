...
class myEnv(gymnasium.Env):
    ...
    def __init__(self,
        ...
        self.X_train, self.X_val, self.y_train, self.y_val = train_test_split(self.X_train,
                                                                              self.y_train,
                                                                              test_size=0.05)
        self.cost_list = [1, 1, 2, 1, 1, 2, 5, 5, 10, 2]
        self.prob_list = [1, 1, 0.5, 1, 1, 0.5, 0.2, 0.2, 0.1, 0.2]
        #self.cost_list= [1]* self.guesser.features_size
        #self.prob_list= [1/(self.guesser.features_size+1)]* (self.guesser.features_size+1)
    ...
    def update_state(self, action, mode):
        ...
        if action < self.guesser.features_size:  # Not making a guess
            if mode == 'training':
                answer = self.X_train.iloc[self.patient, action]
            elif mode == 'val':
                answer = self.X_val.iloc[self.patient, action]
            elif mode == 'test':
                answer = self.X_test.iloc[self.patient, action]
            ...
            question_embedding = self.guesser.map[action]
            # replace the indexes of question_embedding with the answer_vec values
            next_state = np.array(self.state)
            for count, index in enumerate(question_embedding):
